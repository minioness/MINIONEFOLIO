...
import { PROJECT_TECH_ICON_MAP } from "../../constants/projectTechIcon";
...
export default function ProjectCard({ project }: Props) {
  const hasNotion = Boolean(project.notion);

  const handleCardClick = () => {
    if (!hasNotion) return;

    window.open(project.notion, "_blank", "noreferrer");
  };
      ...
      >
        ...
        <div className={styles.techs}>
          {project.techs.map((tech) => (
            <img
              key={tech}
              src={PROJECT_TECH_ICON_MAP[tech]}
              alt={tech}
